...
void normalizeRays(std::vector<std::vector<Vector3D_F>>& vectorArray, PixelMatrix& window)
{
    ...
    for(uint32_t x = 0; x < width; ++x){
        for(uint32_t y = 0; y < hight; ++y){
            ...
            Vector3D_F vec(normalizeX(x, width), normalizeY(y, hight), -1.0);
...
int main()
{
    std::vector<std::vector<Vector3D_F>> vectorArray
            (pixelMatrixWidth_g, std::vector<Vector3D_F>(pixelMatrixHight_g));
